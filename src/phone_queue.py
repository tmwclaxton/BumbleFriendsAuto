...
def _run_job(job: dict) -> tuple[bool, str]:
    ...
    if kind == "refresh_new_friends":
        ...
        return refresh_new_friends_strip()
    if kind == "add_contact":
...
def job_poll_payload(job: dict | None) -> dict:
    ...
    if status == "queued":
        ...
        wait = 20 if kind in {"recapture_all", "fast_scan", "message_new_friends", "grab_photos", "refresh_new_friends"} else 6
    return {
        "ok": True,
        "job_id": job.get("id"),
        "kind": job.get("kind"),
